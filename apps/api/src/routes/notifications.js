...
router.put('/settings', async (req, res) => {
  try {
    const { daily_digest_enabled, daily_digest_time, weekly_report_enabled, weekly_report_day, pipeline_stall_days, recipient_emails } = req.body
    const { rows } = await pool.query(
      `INSERT INTO notification_settings (property_id, daily_digest_enabled, daily_digest_time, weekly_report_enabled, weekly_report_day, pipeline_stall_days, recipient_emails)
       ...
       ON CONFLICT (property_id) DO UPDATE SET
         ...
         updated_at = now()
       RETURNING *`,
      [PROPERTY_ID, daily_digest_enabled, daily_digest_time, weekly_report_enabled, weekly_report_day, pipeline_stall_days || 5, recipient_emails || []]
    )
    res.json(rows[0])
  } catch (err) {
    res.status(500).json({ error: err.message })
